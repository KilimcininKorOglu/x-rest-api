...
func (s *Server) v1Routes() []apiRoute {
	...
	rs := []apiRoute{
		...
		with(route("GET", "/v1/blocks", "Blocked accounts (account-scoped)", []xapi.XUser{}, []openapi.Param{
			{Name: "count", In: "query", Type: "integer", Desc: "Items to return (default 40, max 200)."},
			{Name: "cursor", In: "query", Type: "string", Desc: "Pagination cursor."},
		}), s.blockedAccounts),
		...
		writeRoute(route("POST", "/v1/tweets/{id}/hide", "Hide a reply", writeOK, nil), nil, 200, s.hideReply),
		writeRoute(route("DELETE", "/v1/tweets/{id}/hide", "Unhide a reply", writeOK, nil), nil, 200, s.unhideReply),
